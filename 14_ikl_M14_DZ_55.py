...
from Domashka_14_05.crud_functions import *
from config import *
...
async def sing_up(message):
    print('Поступил запрос на Регистрацию')  # это сообщения для меня (вывода на консоль)
    await message.answer(texts.reg_user, reply_markup=kb)
    await RegistrationState.username.set()  # переход к username
...
async def set_username(message, state):
    ...
    if is_included(message.text):
        ...
        await state.update_data(username=message.text)  # добавляю имя пользователя
        await message.answer('Введите свой email:')
        await RegistrationState.email.set()  # переход к email


@dp.message_handler(state=RegistrationState.email)
async def set_email(message, state):
    # email = message.text
    await state.update_data(email=message.text)  # добавляю почту пользователя
    await message.answer('Введите свой возраст:')
    await RegistrationState.age.set()  # переход к age


@dp.message_handler(state=RegistrationState.age)
async def set_age(message, state):
    # age = message.text
    await state.update_data(age=message.text)  # добавляю возраст пользователя
    data = await state.get_data()  # вытаскиваю данные пользователя
    add_user(data['username'], data['email'], data['age'])  # добавляю в БД
    await message.answer('Регистрация прошла успешна')
    await state.finish()


# ПОКА НЕ НУЖНО (чтобы не мешалось)
